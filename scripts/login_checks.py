...
Standalone adaptation of the main repo's scripts/common/login_checks.py, trimmed
to what this workshop needs (Confluent CLI login + Terraform presence — no
Docker checks, since this workshop has no Docker step). The only
functional change from the original is `ensure_confluent_login`'s fallback
credentials path, updated to read gemini-workshop's own credentials.env instead
of the main repo's root one.
"""
...
def check_confluent_login() -> bool:
    ...
    a valid session even in an org with zero environments, which stdout parsing
    would misread as logged out.
    """
    try:
...
def ensure_confluent_login(creds: dict | None = None, creds_file=None, interactive: bool = False) -> bool:
    ...
    only) prompt once and save. Returns True if logged in, False otherwise.
    """
    ...
    if creds is None:
        ...
        creds = dict(dotenv_values(project_root() / "credentials.env"))
