# Flat outputs consumed by gemini-workshop/scripts/cli.py (credential card +
# seeding). Deliberately identical in shape to terraform/self-service/outputs.tf
# so `_card_fields` in scripts/workshop/creds.py needs no changes to read them.
